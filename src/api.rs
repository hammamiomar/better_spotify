...
// #[server(ExecuteShuffleAndCreate)]
// pub async fn execute_shuffle_and_create(original_playlist_name: String, user_id: String, mut track_uris: Vec<String> ) -> Result<NewPlaylistDetails, ServerFnError> {
//         tracing::info!(
//             "API: Executing shuffle for '{}', user: {}, tracks_count: {}",
...
//                 "No track URIs provided to shuffle.".to_string(),
//             ));
//         }

//         // 1. Shuffle the provided track URIs
//         let mut rng = thread_rng();

//         track_uris.shuffle(&mut rng);
...
//         struct CreatePlaylistPayload<'a> {
//             name: &'a str,
//             public: bool,
//             description: String,
//         }
...
//             public: false,
//             description: format!(
//                 "A true random shuffle of '{}'. Generated by Hotdogify!",
//                 original_playlist_name
//             ),
//         };
//         let create_playlist_url =
...
//                         Ok(data) => data, // This `data` is then assigned to created_playlist_data
//                         Err(e) => {
//                             // Construct and return ServerFnError explicitly
...
//                     tracing::error!("API: Spotify error creating playlist ({}) : {}", status, err_text);
...
//             Err(e) => {
//                 return Err(ServerFnError::ServerError(format!(
//                     "API: Network error creating playlist: {}",
//                     e
//                 )))
//             }
//         };
...
//         tracing::info!(
//             "API: New playlist created '{}' (ID: {})",
//             new_playlist_name,
//             new_playlist_id
//         );

//         // 3. Add Shuffled Tracks to the New Playlist
//         if !track_uris.is_empty() {
//             let add_tracks_url_base =
//                 format!("https://api.spotify.com/v1/playlists/{}/tracks", new_playlist_id);
...
//                             let status = response.status();
//                             let err_text = response.text().await.unwrap_or_default();
//                              tracing::error!("API: Spotify error adding tracks ({}) : {}", status, err_text);
//                             return Err(ServerFnError::ServerError(format!(
//                                 "Spotify API error ({}) adding tracks",
//                                 status
//                             )));
//                         }
//                         tracing::info!(
//                             "API: Successfully added a chunk of {} tracks.",
//                             chunk_of_uris.len()
//                         );
//                     }
//                     Err(e) => {
//                         return Err(ServerFnError::ServerError(format!(
//                             "API: Network error adding tracks: {}",
//                             e
//                         )))
//                     }
//                 }
//                 if track_uris.len() > 100 && chunk_of_uris.len() == 100 {
//                     tokio::time::sleep(tokio::time::Duration::from_millis(250)).await;
//                 }
//             }
//         }
//         tracing::info!(
//             "API: All tracks added to new playlist: {}",
//             new_playlist_name
//         );
